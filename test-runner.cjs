...
class OptimizedMCPTest {
  ...
  async executeClaudeCommand(command, workingDir, timeout) {
    return new Promise((resolve, reject) => {
      ...
      const commandName = args.shift(); // Remove 'claude' from args
  ...
  async runTestCommand(workingDir, test, useMcp) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        const output = await this.executeClaudeCommand(claudeCmd, workingDir, timeout);
